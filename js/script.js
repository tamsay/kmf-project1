let getSelectChoice=(selectType, valueSpan, checkIcon)=>{
    ...
    valueSpan.innerText = 'Select Operation'
...
let displayOperationTypes=(()=>{
    ...
    let defaultCheckIcon = operationType.querySelector('.menu li .iconify')
        ...
        getSelectChoice(operationTypes, operationValue, defaultCheckIcon);
})()
...
let createOperationBtn = document.querySelector('#add-new-operation')
...
let createTodoItem =(value, status, date)=>{
    ...
    let secondSection = document.createElement('div')
        secondSection.classList = 'secondSection col-12 col'
    ...
    let deleteDiv = document.createElement('div')
        deleteDiv.classList = 'deleteDiv btn flex-row'
    ...
    rowWrapper.appendChild(editDiv)
    rowWrapper.appendChild(deleteDiv)
    ...
    let todoContent = editBtn.parentElement.parentElement.parentElement.parentElement.firstChild;
...
createOperationBtn.addEventListener('click',()=>{
    todoInput.value = ''
    setTimeout(() => {
        todoInput.focus();
    }, 700);
